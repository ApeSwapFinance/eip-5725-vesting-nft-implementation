...
async function runNormalTests(
  ...
  args.testFiles = args.testFiles.filter((file: string) =>
...
async function runDeployTests(
  ...
  if (args.fork)
    throw Error(
      "The 'fork' option is invalid when testing deployments on livenetwork"
    )
  args.testFiles = args.testFiles.filter((file: string) =>
    file.endsWith('.deploy.ts')
  )
  await run(args)
}
...
async function runForkTests(
  ...
  args.testFiles = args.testFiles.filter((file: string) =>
    file.endsWith('.fork.ts')
  )

  const forkingNetworkName = Object.keys(hre.config.networks).find(
    (networkName) => networkName === args.fork
  )
  if (!forkingNetworkName)
    throw Error(`Could not find a config for network ${args.fork} to be forked`)

  const forkingNetworkConfig = hre.config.networks[
    forkingNetworkName
  ] as HttpNetworkConfig
  if (!forkingNetworkConfig.url)
    throw Error(
      `Could not find a RPC url in network config for ${forkingNetworkName}`
    )
...
export function getForkedNetwork(hre: HardhatRuntimeEnvironment): string {
  ...
  const network = Object.entries(hre.config.networks).find(
    ([, networkConfig]) => {
      const httpNetworkConfig = networkConfig as HttpNetworkConfig
      return (
        httpNetworkConfig.url && httpNetworkConfig.url === config?.forking?.url
      )
    }
  )
